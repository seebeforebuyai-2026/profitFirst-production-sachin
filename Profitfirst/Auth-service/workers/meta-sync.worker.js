...
// 🟢 FIX 1: Define sleep function to prevent crash
const sleep = (ms) => new Promise(r => setTimeout(r, ms));

const pollQueue = async () => {
  console.log("🚀 [MetaWorker] Monitoring Ads Queue...");
  while (!isShuttingDown) {
    try {
      ...
      await sqsClient.send(new DeleteMessageCommand({ 
        QueueUrl: metaQueueUrl, ReceiptHandle: message.ReceiptHandle 
      }));
    } catch (e) { 
...
const processMetaSync = async (job) => {
  ...
  try {
    ...
    const token = encryptionService.decrypt(integration.accessToken);
    ...
    if (mode === "incremental") {
        // 7-day sliding window for attribution accuracy
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(today.getDate() - 7);
        startStr = sevenDaysAgo.toISOString().split('T')[0];
        endStr = today.toISOString().split('T')[0];
    } else {
        startStr = sinceDate.split('T')[0];
        endStr = today.toISOString().split('T')[0];
    }

    const dirtyDates = new Set(affectedDates);

    let nextUrl = `https://graph.facebook.com/v20.0/${adAccountId}/insights?access_token=${token}&time_range=${JSON.stringify({ since: startStr, until: endStr })}&fields=spend,impressions,clicks,reach,actions,action_values,inline_link_click_ctr&time_increment=1&limit=50`;

    while (nextUrl) {
        // 🟢 FIX 2: Implementation of Rate Limit Protection
        await sleep(500); 
        ...
        for (const day of dailyData) {
            try {
                ...
                // --- Step A: S3 Raw Backup (Essential Rule #4) ---
                ...
                await newDynamoDB.send(new PutCommand({ 
                    ...
                    Item: { 
                        ...
                        entityType: "ADS", 
                        date: dateKey,
                        spend: Number(day.spend || 0), // Only spend as requested
...
async function markSyncComplete(merchantId, mode, sinceDate, finalAffectedDates) {
  try {
    ...
    // ✉️ Baton Pass to Shiprocket with the UPDATED Affected Dates
    await sqsClient.send(new SendMessageCommand({ 
      ...
      MessageBody: JSON.stringify({ 
          ...
          sinceDate, 
          mode, 
          currentAffectedDates: finalAffectedDates // Growing list
      }) 
    }));
    
    console.log(`✅ [Meta] Incremental Done. Passing ${finalAffectedDates.length} dirty dates to Shiprocket.`);
