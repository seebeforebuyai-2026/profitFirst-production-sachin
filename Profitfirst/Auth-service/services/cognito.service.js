 ...
 * Includes signup, login, password management, token operations, and OAuth and new ssologin
 */
...
class CognitoService {
    ...
    try {
      ...
      if (error.message && error.message.includes("name.formatted")) {
        return {
          success: false,
          error:
            "Cognito User Pool configuration error: name.formatted is required but cannot be set via API. Please go to AWS Cognito Console → Your User Pool → Sign-up experience → Attributes and make name.formatted optional or remove it. See SOLUTION.md for detailed instructions.",
        };
      }
  ...
  async verifyEmail(email, otp) {
    try {
      ...
      if (error.name === "CodeMismatchException") {
        console.error(
          `   Reason: OTP code does not match what Cognito expects`,
        );
        return {
          success: false,
          error:
            "Invalid verification code. The code you entered does not match. Please check your email and try again.",
        };
        ...
        return {
          success: false,
          error:
            'Verification code has expired. Please click "Resend Code" to get a new one.',
        };
        ...
        return {
          success: false,
          error: "User is already verified. Please try logging in.",
        };
        ...
        return {
          success: false,
          error: "This email is already verified with another account.",
        };
      }
  ...
  async resendOTP(email) {
    try {
      ...
      console.log(
        `  Delivery method: ${result.CodeDeliveryDetails?.DeliveryMedium}`,
      );
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          error:
            "Cognito configuration error: USER_PASSWORD_AUTH flow not enabled. Go to AWS Cognito Console → User Pool → App clients → Edit → Enable USER_PASSWORD_AUTH in Authentication flows.",
        };
      } else if (error.name === "UserNotConfirmedException") {
        return {
          success: false,
          error: "Email not verified. Please verify your email with OTP.",
        };
      } else if (error.name === "NotAuthorizedException") {
  ...
  async forgotPassword(email) {
    try {
      ...
      console.log(
        `  Delivery method: ${result.CodeDeliveryDetails?.DeliveryMedium || "EMAIL"}`,
      );
  ...
  async markEmailAsVerified(email) {
    try {
      ...
      if (error.name === "UserNotFoundException") {
        console.error(
          "User not found in Cognito. They may need to sign up first.",
        );
      } else if (error.name === "NotAuthorizedException") {
        console.error(
          "IAM permissions missing. Need cognito-idp:AdminUpdateUserAttributes permission.",
        );
      }
  ...
  async adminCreateUser(email, firstName, lastName) {
    try {
      const {
        AdminCreateUserCommand,
      } = require("@aws-sdk/client-cognito-identity-provider");
      const command = new AdminCreateUserCommand({
        ...
        MessageAction: "SUPPRESS", // OTP email mat bhejo — merchant ko pata nahi hoga
      });
      const result = await cognito.send(command);
      return { success: true, data: result.User };
    } catch (error) {
      // Agar user already exist karta hai toh error mat throw karo
  ...
  async adminSetPasswordAndAuth(email) {
    try {
      // Step 1: Random strong password generate karo (merchant ko kabhi nahi dikhega)
      const crypto = require("crypto");
      const tempPassword =
        "Tmp" + crypto.randomBytes(12).toString("hex") + "Aa1!";

      // Step 2: Password set karo
      const {
        AdminSetUserPasswordCommand,
        AdminInitiateAuthCommand,
      } = require("@aws-sdk/client-cognito-identity-provider");
      ...
      await cognito.send(
        new AdminSetUserPasswordCommand({
          ...
          Permanent: true, // Permanent password — FORCE_CHANGE_PASSWORD state se bacho
        }),
      );

      // Step 3: AdminInitiateAuth se real Cognito tokens lo
      ...
      console.error("adminSetPasswordAndAuth error:", error.message);
      return { success: false, error: error.message };
    }
  }
}
