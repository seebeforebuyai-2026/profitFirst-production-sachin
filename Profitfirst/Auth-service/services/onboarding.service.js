...
class OnboardingService {
  ...
  async getOnboardingStatus(merchantId) {
    try {
      ...
      const productsResult =
        await dynamodbService.getProductsByMerchant(merchantId);
      const variantsResult =
        await dynamodbService.getVariantsByMerchant(merchantId);
  ...
  async updateStep1BusinessInfo(merchantId, businessData) {
    try {
      ...
      const result = await dynamodbService.updateUserProfileOnboarding(
        merchantId,
        updates,
      );
    ...
    try {
      ...
      if (shopifyData.accessToken) {
        ...
        if (existingIntegration.success) {
          ...
          console.log(
            `🔄 Shopify integration exists, updating for merchant: ${merchantId}`,
          );
          integrationResult = await dynamodbService.updateIntegration(
            ...
            "shopify",
            {
              ...
              accessToken: encryptionService.encrypt(shopifyData.accessToken), // TODO: Encrypt this
          ...
          console.log(
            `➕ Creating new Shopify integration for merchant: ${merchantId}`,
          );
        ...
        await dynamodbService.updateUserProfileOnboarding(
          merchantId,
          profileUpdates,
        );
        ...
        return {
          ...
          data: {
            ...
            shopifyStore: shopifyData.shopifyStore,
            message:
              "Store URL validated. Please install the app via external service.",
        ...
        switch (status) {
          case 401:
            return {
              success: false,
              error:
                "Invalid access token. Please check your Shopify credentials.",
            };
          case 403:
            return {
              success: false,
              error:
                "Access denied. Please ensure your app has the required permissions.",
            };
          case 404:
            return {
              success: false,
  ...
  async updateStep3MetaIntegration(merchantId, metaData) {
    try {
      ...
      if (existingIntegration.success) {
        console.log(
          `🔄 Meta integration exists, updating for merchant: ${merchantId}`,
        );
        integrationResult = await dynamodbService.updateIntegration(
          merchantId,
          "meta",
        ...
        console.log(
          `➕ Creating new Meta integration for merchant: ${merchantId}`,
        );
        integrationResult = await dynamodbService.createIntegration({
          merchantId: merchantId,
          platform: "meta",
  ...
  async updateStep4ShiprocketIntegration(merchantId, shiprocketData) {
    try {
      ...
      try {
        console.log(
          `🚀 Authenticating with Shiprocket for: ${shiprocketData.email}`,
        );
        ...
        console.log(
          `✅ Shiprocket Authentication Successful! Token generated.`,
        );
      } catch (apiError) {
        ...
        return {
          success: false,
          error:
            "Invalid Shiprocket credentials. Please check your email and password.",
        };
      }
      ...
      if (existingIntegration.success) {
        console.log(
          `🔄 Shiprocket integration exists, updating for merchant: ${merchantId}`,
        );
        integrationResult = await dynamodbService.updateIntegration(
          merchantId,
          "shiprocket",
        ...
        console.log(
          `➕ Creating new Shiprocket integration for merchant: ${merchantId}`,
        );
        integrationResult = await dynamodbService.createIntegration({
          merchantId: merchantId,
          platform: "shiprocket",
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          integrations: integrationsResult.success
  ...
  async updateOnboardingStep(merchantId, userId, step, stepData = {}) {
    console.warn(
      "⚠️  updateOnboardingStep is deprecated. Use specific step methods instead.",
    );
