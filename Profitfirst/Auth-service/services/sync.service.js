...
class SyncService {
  async startInitialSync(merchantId) {
    try {
      ...
      console.log(
        `🚀 [Relay Race Start] Shopify job sent to specific queue for ${merchantId}`,
      );
      return { success: true };
    } catch (error) {
      throw error;
  ...
  async getSyncStatus(merchantId) {
    ...
    result.Items.forEach(
      (i) => (status[i.SK.replace("SYNC#", "").toLowerCase()] = i),
    );
    return {
      success: true,
      shopify: status.shopify || { percent: 0 },
      meta: status.meta || { percent: 0 },
      shiprocket: status.shiprocket || { percent: 0 },
    };
  }
  ...
  async checkAndUnlockDashboard(merchantId) {
    ...
    await newDynamoDB.send(
      new UpdateCommand({
        ...
        UpdateExpression:
          "SET dashboardUnlocked = :true, initialSyncCompleted = :true, updatedAt = :t",
        ExpressionAttributeValues: {
          ":true": true,
