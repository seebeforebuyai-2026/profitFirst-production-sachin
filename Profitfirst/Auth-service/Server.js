...
const authLimiter = rateLimit({
  ...
  skip: (req) => {
    ...
    return url.includes("/sso-verify") || url.includes("/shopify-sso");
  },
});
...
app.get("/", (req, res) => {
  ...
  if (req.query.error) {
    return res.status(400).json({
      ...
      error_description:
        req.query.error_description || "OAuth authentication failed",
    });
  }
  ...
  app.get("/api/auth/oauth/config", (_req, res) => {
    ...
    if (!config.isConfigured) {
      config.warnings.push(
        "OAuth not fully configured. Set COGNITO_DOMAIN, COGNITO_REDIRECT_URI, and COGNITO_CLIENT_ID in .env",
      );
    }

    if (
      config.redirectUri &&
      !config.redirectUri.includes("/api/auth/oauth/callback")
    ) {
      config.warnings.push(
        "COGNITO_REDIRECT_URI should include /api/auth/oauth/callback path",
      );
      config.warnings.push(`Current: ${config.redirectUri}`);
      config.warnings.push(
        `Expected: http://localhost:3000/api/auth/oauth/callback`,
      );
    }

    if (config.cognitoDomain && config.cognitoDomain.includes("your-domain")) {
      config.warnings.push(
        'COGNITO_DOMAIN contains placeholder "your-domain". Update with actual Cognito domain.',
      );
    }
...
app.use((err, req, res, _next) => {
  ...
  console.error(
    `[${new Date().toISOString()}] Error on ${req.method} ${req.path}:`,
    {
      message: err.message,
  ...
  if (err.type === "entity.too.large") {
    return res
      .status(413)
      .json({ error: "Request body too large. Maximum size is 10KB." });
  }
