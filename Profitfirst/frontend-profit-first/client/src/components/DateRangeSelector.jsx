...
const DateRangeSelector = ({ onApply, initialRange }) => {
  ...
  const handleDateChange = (item) => {
    ...
    // 🟢 Production Rule: Prevent future date selection
  ...
  const handleApply = () => {
    // 🟢 CRITICAL: Format dates to YYYY-MM-DD before sending to API
    // This ensures the frontend matches the SUMMARY#SK in DynamoDB exactly.
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#161616] text-white rounded-xl shadow-2xl border border-gray-800 overflow-hidden w-full max-w-2xl">
      ...
      <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-gray-800 p-2 space-y-1">
        <div className="px-3 py-2 text-xs font-bold text-gray-500 uppercase tracking-widest">Quick Select</div>
      ...
      <div className="p-4 flex flex-col items-center bg-[#1a1a1a]">
        <style>{`
          .rdrCalendarWrapper { background: transparent !important; color: #fff !important; }
          .rdrDayNumber span { color: #ddd !important; }
          .rdrSelected, .rdrInRange, .rdrStartEdge, .rdrEndEdge { background: #22c55e !important; }
          .rdrDayInPreview { border-color: #22c55e !important; }
          .rdrMonthAndYearPickers select { color: #000 !important; }
          .rdrDayPassive { opacity: 0.3 !important; }
          .rdrDayToday .rdrDayNumber span:after { background: #22c55e !important; }
        `}</style>
        ...
        <DateRange
          ...
          ranges={state}
          maxDate={new Date()} // Locked to Today
          rangeColors={["#22c55e"]}
