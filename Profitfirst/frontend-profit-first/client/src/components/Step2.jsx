...
const Step2 = ({ onComplete }) => {
  ...
  const getStatusDisplay = () => {
    switch (connectionStatus.step) {
      case "connecting":
        return {
          icon: "🔗",
          message: "Opening Shopify installation...",
          progress: 25,
        };
      case "installing":
        return {
          icon: "📱",
          message: "Please install the app in Shopify",
          progress: 50,
        };
      case "polling":
        return {
  ...
  const showSmartToast = (step) => {
    switch (step) {
      ...
      case "installing":
        toast.info(
          "📱 Please install our app in the Shopify tab, then we'll detect it automatically!",
          { autoClose: false },
        );
        break;
      case "polling":
        toast.info("🔍 Checking for installation... This may take a moment.", {
          autoClose: 5000,
        });
        break;
      case "verified":
        toast.success("✅ Connection verified! Advancing to next step...", {
          autoClose: 3000,
        });
        break;
      case "timeout":
        toast.warning(
          "⏰ Taking longer than expected? Click 'Verify Connection' when ready, or try connecting again.",
          { autoClose: 10000 },
        );
        break;
    }
  ...
  const startPollingForToken = () => {
    ...
    setConnectionStatus({
      step: "polling",
      message: "Checking for installation...",
      progress: 75,
    });
    ...
    timeoutRef.current = setTimeout(() => {
      if (connectionStatus.step === "polling") {
        setConnectionStatus({
          step: "installing",
          message: "Manual verification available",
          progress: 50,
        });
        showSmartToast("timeout");
      }
  ...
  const handleAutoVerify = async (accessToken) => {
    try {
      ...
      toast.error(
        "❌ Auto-verification failed. Please click 'Verify Connection'.",
      );
    }
  };
  ...
  const handleConnect = async () => {
    ...
    if (!shopifyRegex.test(correctedStoreUrl)) {
      return toast.error(
        "Invalid Shopify store URL format. Example: yourstore.myshopify.com",
      );
    }
    ...
    try {
      ...
      if (response.data.success && response.data.authUrl) {
        console.log(
          "🔗 Opening external OAuth service:",
  ...
  const handleVerifyConnection = async () => {
    ...
    try {
      ...
      console.log(
        "✅ Token received:",
        tokenResponse.data.accessToken.substring(0, 20) + "...",
      );
    ...
    <div className="min-h-screen bg-[#101218] text-white flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <main className="w-full max-w-6xl flex flex-col lg:flex-row items-center justify-between px-12 gap-16">
        ...
        <div className="bg-[#1E1E1E] border-[#1E1E1E] rounded-[20px] p-10 shadow-lg w-full max-w-md">
            ...
            >
              ...
              connectionStatus.step !== "connected" ? (
                <>
                  <PulseLoader
                    size={8}
                    color={buttonConfig.primary ? "black" : "white"}
                  />
                  {buttonConfig.text}
                </>
              ) : (
          ...
          {connectionStatus.step === "installing" && (
            <div className="mt-4 text-center text-xs text-gray-500">
              🔍 We're checking for your app installation automatically.
              <br />
        ...
        <div className="bg-[#141617] rounded-[20px] w-full max-w-xl shadow-md overflow-hidden border border-gray-800">
          ...
          >
            <source
              src="https://res.cloudinary.com/dqdvr35aj/video/upload/v1780316546/Shopify_Integration_r3oio4.mp4"
              type="video/mp4"
            />
          </video>
          ...
          <div className="p-4">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Follow this walkthrough to connect your Shopify store and complete
              onboarding.
            </p>
          </div>
        </div>
