...
const Products = () => {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const updatedVariants = isLoadMore ? [...allVariantsList, ...newVariants] : newVariants;
  ...
  const bulkApply = (productId, value) => {
    const product = products.find((p) => p.productId === productId);
    if (!product || !value) {
      toast.warn("Please enter a cost first");
      return;
    }
    const updatedCogs = { ...cogs };
    product.variants.forEach((v) => {
      updatedCogs[v.variantId] = Number(value);
    });
    setCogs(updatedCogs);
    toast.info(`Applied cost to all ${product.productName} variants`);
  };
      ...
      try {
        ...
        if (response.data.success) {
          setSyncStatus({
            ...
            shiprocket: response.data.shiprocket || { status: "pending", percent: 0 },
          });
  ...
  const handleCogsChange = (variantId, value) => {
    setCogs((prev) => ({ ...prev, [variantId]: value === "" ? "" : Number(value) }));
  };
  ...
  const handleSaveCogs = async () => {
    ...
    const variants = Object.entries(cogs).map(([variantId, costPrice]) => ({
      variantId, costPrice: Number(costPrice),
    }));

    try {
      const response = await axiosInstance.post("/products/save-cogs", { variants });
      if (response.data.success) {
    ...
    } catch (error) { toast.error("Failed to save costs"); }
  };
  ...
  const startSync = async () => {
    ...
    } catch (error) { toast.error("Failed to start background sync"); }
  ...
  if (loading && products.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#0D1D1E] gap-4">
        ...
        <p className="text-green-500 text-sm animate-pulse">Initializing product catalog...</p>
      </div>
    );
  }
    ...
    <div className="p-6 text-white max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Set your manufacturing costs to calculate accurate net profit.</p>
        </div>
        <button onClick={() => fetchProducts()} className="p-2 hover:bg-white/5 rounded-full transition-colors text-gray-400">
           <FiRefreshCw size={20} />
        </button>
      </div>
      ...
      <div className="p-4 bg-[#1E1E1E] rounded-xl border border-gray-800 grid grid-cols-1 md:grid-cols-3 gap-6">
        <ProgressBar label="Shopify" color="bg-green-500" percent={syncStatus.shopify.percent} />
        <ProgressBar label="Meta Ads" color="bg-blue-500" percent={syncStatus.meta.percent} />
        <ProgressBar label="Shiprocket" color="bg-purple-500" percent={syncStatus.shiprocket.percent} />
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-3 bg-green-500/5 border border-green-500/20 p-2 px-4 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <button onClick={applyGlobalFormula} className="px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-xs font-bold rounded shadow-lg transition-all">Apply</button>
          </div>
        </div>
      </div>
              ...
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product) =>
                  product.variants?.map((variant, idx) => (
                    <tr key={variant.variantId} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="p-4">
                        {idx === 0 && (
                          <img
                            src={variant.productImage || "https://via.placeholder.com/40?text=📦"}
                            alt=""
                      ...
                      <td className="p-4">
                        {idx === 0 && (
                          <div>
                            <div className="font-bold text-gray-100">{product.productName}</div>
                            {product.variants.length > 1 && (
                               <button
                                 onClick={() => bulkApply(product.productId, cogs[variant.variantId])}
                                 className="text-[9px] text-green-400 hover:text-green-300 font-bold mt-1 uppercase tracking-tighter"
                               >
                                 Bulk Apply to {product.variants.length} Variants
                               </button>
                            )}
                          </div>
                        )}
                      </td>
                      <td className="p-4 text-gray-400 text-sm">{variant.variantName}</td>
                      <td className="p-4 text-gray-300 font-mono">₹{variant.salePrice?.toLocaleString()}</td>
                      <td className="p-4">
                        <div className="flex items-center gap-2 group-hover:translate-x-1 transition-transform">
                          ...
                          <input
                            ...
                            onChange={(e) => handleCogsChange(variant.variantId, e.target.value)}
                ...
                <tr>
                  <td colSpan="5" className="p-20 text-center flex flex-col items-center gap-3">
                    ...
                    <p className="text-gray-500">{loading ? "Loading items..." : "No matching products found."}</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="flex justify-end pt-4 pb-10">
        <button
          ...
          disabled={Object.values(cogs).some((v) => v === "" || v <= 0) || syncingOrders}
          className="group px-10 py-4 bg-green-500 hover:bg-green-600 disabled:opacity-30 disabled:cursor-not-allowed text-black font-black uppercase tracking-widest rounded-2xl transition-all shadow-[0_0_20px_rgba(34,197,94,0.3)] hover:shadow-green-500/50 flex items-center gap-3"
        >
...
const ProgressBar = ({ label, color, percent }) => (
  <div>
    ...
    <div className="h-1.5 bg-gray-900 rounded-full overflow-hidden border border-white/5">
      <div className={`h-full ${color} transition-all duration-1000 ease-in-out`} style={{ width: `${percent}%` }} />
    </div>
  </div>
);
