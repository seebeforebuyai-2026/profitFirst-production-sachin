...
const BusinessExpenses = () => {
  ...
  useEffect(() => {
    // 🟢 If user is already unlocked, or if they land here by mistake, send to dashboard
    if (profile?.dashboardUnlocked === true) {
      navigate("/dashboard");
    }
  }, [profile?.dashboardUnlocked, navigate]);
  // 🟢 POLL SYNC STATUS: Start polling only after expenses are saved
  useEffect(() => {
    let interval;
    if (profile?.expensesCompleted) {
      const poll = async () => {
        try {
          ...
          if (response.data.success) {
            ...
            if (allDone) {
              console.log(
                "🏁 All syncs finished. Waiting for final profit calculation...",
              );
              // 🟢 Wait 2 seconds for the Summary Calculator to finish before checking the lock
              setTimeout(() => {
                fetchProfile();
              }, 2000);
            }
  ...
  }, [profile?.expensesCompleted, fetchProfile]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success("✅ Expenses saved! Starting 1-year data sync...");
        setHasChanges(false);

        // Update global state: This will trigger the Progress Bars to show
    ...
    <div className="p-6 text-white space-y-6 max-w-6xl mx-auto pb-32 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {!profile?.expensesCompleted && (
          ...
          >
            {isSaving ? "Initializing..." : "Finalize & Sync Dashboard →"}
          </button>
        )}
      </div>

      {/* 🟢 SYNC PROGRESS SECTION (Only visible after clicking save) */}
      {profile?.expensesCompleted && (
        <div className="bg-[#1a1a1a] border-2 border-green-500/30 p-8 rounded-[2rem] shadow-2xl space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold">
                Syncing 1-Year Financial Data
              </h3>
              <p className="text-gray-500 text-xs">
                Processing Shopify, Meta, and Shiprocket. Please do not close
                this page.
              </p>
            </div>
            <div>
              <h3 className="text-xl font-bold">
                ...
                syncStatus.meta.percent === 100
                  ? "🎉 Finalizing Profit Math..."
                  : "Syncing 60-Day History"}
              </h3>
              <p className="text-gray-500 text-xs">
                {syncStatus.shopify.percent === 100 &&
                syncStatus.meta.percent === 100
                  ? "Almost ready! We are generating your final dashboard reports."
                  : "Processing Shopify, Meta, and Shiprocket. Please do not close this page."}
              </p>
            </div>
          </div>
      ...
      {/* INPUT GRID (Disabled if sync already started to prevent errors) */}
      <div
        className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${profile?.expensesCompleted ? "opacity-50 pointer-events-none" : ""}`}
      >
        ...
        <div className="bg-[#161616] border border-gray-800 rounded-2xl p-6 space-y-5">
          ...
          <InputField
            ...
            onChange={(val) => handleInputChange("staffFees", val)}
            icon="₹"
          />
        </div>
...
const SyncProgressBar = ({ label, percent, color }) => (
  <div className="space-y-2">
    ...
    <div className="h-2 bg-gray-800 rounded-full overflow-hidden border border-white/5">
      <div
        className={`h-full ${color} transition-all duration-1000 ease-in-out shadow-[0_0_10px_rgba(34,197,94,0.3)]`}
        style={{ width: `${percent}%` }}
      />
    </div>
    {percent === 100 && (
      <div className="flex items-center gap-1 text-green-500 text-[10px] font-bold uppercase tracking-tighter">
        <FiCheckCircle size={10} /> Data Ready
      </div>
    )}
  </div>
);
